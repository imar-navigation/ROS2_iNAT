...
using XComMessages_MagField = xcom::MessageHandler<XCOMmsg_MAGDATA>;
using XComParameters_MagField = xcom::ParameterHandler<XCOMParEKF_MAGATTAID>;

class MagneticField : public XComMessages_MagField, XComParameters_MagField, xcom::ResponseHandler  // , xcom::CommandHandler
{
public:
    MagneticField(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
...
private:
    ...
    void init();
    void setParData(const XCOMParEKF_MAGATTAID& param);
    void updateMAGDATA(const XCOMmsg_MAGDATA& msg);
    void publish();
    ...
    std::atomic_uint32_t par_MAGATTAID_age_ = ATOMIC_VAR_INIT(0);
    uint32_t par_MAGATTAID_c_ = 0;
    uint16_t msg_MAGDATA_frq_ = 0;
    std::atomic_uint32_t msg_MAGDATA_age_ = ATOMIC_VAR_INIT(0);
    uint32_t msg_MAGDATA_c_ = 0;
