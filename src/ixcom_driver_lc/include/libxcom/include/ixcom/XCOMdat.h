...
* 	CHANGE HISTORY:
*
* 	1.0 - Aug 26, 2014 : T. Schneider - File created
*---------------------------------------------------------------------
...
enum XComMessageID {
    ...
    XCOM_MSGID_IMU2RAW              = 0x7D,    /**< 2nd IMU data - calibrated */ 
...
enum XComParameterID {
    ...
    XCOMPAR_PARSYS_GITSHA           = 25,   /**< This parameter holds the git hash of the current commit */
    XCOMPAR_PARSYS_GITBRANCH        = 26,   /**< This parameter holds the git branch name of the source code */
    ...
    XCOMPAR_PARXCOM_LOGLIST2        = 917,    /**< This parameter contains 16 structs, each holding a pair of message ID with a divider for
                                                   ...
                                                   a log-list of a specific iX- COM channel. If the log-list should be changed, please use the
    ...
    XCOMPAR_PARXCOM_CLIENT          = 919,    /**< This parameter is for configuring an iXCOM client and its client logs. The maximum supported
                                                   number of clients is currently eight. */
...
enum PluginIDs {
    ...
    XCOM_PLUGINID_GREENLIGHT_LOCAL          = XCOM_MSGID_GREENLIGHTLOCAL,       /**< Oebb-Verschub: GREENLIGHTLOCAL message as plugin data */
    XCOM_PLUGINID_GREENLIGHT_SMARTPHONE     = XCOM_MSGID_GREENLIGHTSMARTPHONE,  /**< Oebb-Verschub: GREENLIGHTSMARTPHONE message as plugin data */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint8_t imu_temperature_axis;   /**< IMU temperature axis identifier (see XCOMMSG_RAWDATA_IMUAXIS_*)  */
    union {
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;  /**< XCOM header */
    float omg_dot[3];   /**< Angular acceleration around x, y and z [rad/s2 ] */
    XCOMFooter footer;
} XCOMmsg_OMGDOT;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    double latitude;                        /**< INS longitude (±π/2) in [rad] */
    double longitude;                       /**< INS latitude (±π) in [rad] */
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    float raw_values[3];        /**< Field values (nT) */
    uint8_t bit_error;          /**< Built-In Self Test result */
 ...
 * In addition to the standard deviations reported in the EKFSTDDEV, estimated sensor nonorthogonality and misalignment standard
...
/**
 * The EKFSTDDEV3 message contains the estimated sensor errors as well as the current standard deviations of position parameters and of
 * orientation angles (RPY) by the extended Kalman Filter. Furthermore, it displays the accuracy of other estimated values.
 * In addition to the standard deviations reported in the EKFSTDDEV, estimated sensor nonorthogonality and misalignment standard
 * deviations are included in this log.
 *
...
/**
 * The EKFSENSORERR2 message contains the sensor errors estimated by the extended Kalman filter. In addition to the sensor errors reported
 * in the EKFSENSORERR, estimated sensor non orthogonalities and misalignments are included in this log.
 * #domain: public
 * #rate: ekf
 */
typedef struct XCOM_STRUCT_PACK {
...
/**
 * This message contains the ADC measurements of the INS.
 * #domain: public
 * #rate: event
 * #name: XCOMmsg_POWER
 */
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                                  /**< XCOM header */
    uint32_t status;                                    /**< Status information code */
    int8_t last_msg[XCOMMSG_NTRIPSTATUS_BUFFERSIZE];    /**< Status message */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    double q_nb[4];      /**< Computation frame to NED quaternion */
    float acc[3];        /**< Calibrated acceleration along IMU x-, y- and z-axis in [m/s2] */
    float omg[3];        /**< Calibrated angular rate along IMU x-, y- and z-axis in [rad/s] */
    ...
    float att_stddev[3]; /**< Standard deviation of error tilt angles around north, east, and down axis. Down axis tilt stddev.
                              is equivalent to yaw angle stddev. [rad] */
    XCOMFooter footer;
} XCOMmsg_MVCSLAVE;

/**
 * This message contains the MCV data
 * #domain: hidden
 * #rate: event
...
typedef struct XCOM_STRUCT_PACK {
    ...
    float att_std_dev[3];  /**< Standard deviation of error tilt angles around  north, east, and down axis. Down axis
...
typedef struct XCOM_STRUCT_PACK {
    ...
    double pos_ecef[3]; /**< Satellite position for x-, y- and z- axis within ECEF frame in [m] */
    double vel_ecef[3]; /**< Satellite velocity for x-, y- and z- axis within ECEF frame in [m/s] */
    float cn0[3];       /**< Satellite carrier to noise ratio in dB-Hz for L1, L2 and L5 carrier */
 ...
 * Each item consists of a description, a related status and a float value.
 * #domain: public
 * #rate: gnss
 */
typedef struct XCOM_STRUCT_PACK {
...
/**
 * GNSS raw data stream in proprietary binary protocol (depends on internal receiver type).
 * Please note that the message length is variable.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                               /**< XCOM header */
    uint8_t payload_buffer[XCOM_MAX_MESSAGE_LENGTH]; /**< Generic XCOM payload buffer */
    XCOMFooter footer;                               /**< XCOM footer */
} XCOMmsg_GNSSSTREAM;

/**
 * GNSS correction data in proprietary binary protocol (depends on the selected data format).
 * Please note that the message length is variable
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                               /**< XCOM header */
...
/**
 * GNSS satellite status information.
 * Please note that the message length is variable.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                               /**< XCOM header */
...
/**
 * This message contains the ABD binary message
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                               /**< XCOM header */
    uint8_t payload_buffer[XCOM_MAX_MESSAGE_LENGTH]; /**< XCOM payload buffer */
...
/**
 * External sensor raw data stream.
 * Please note that the message length is variable.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                               /**< XCOM header */
...
/**
 * Monitor messages (for debugging purpose only).
 * Please note that the message length is variable.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                               /**< XCOM header */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    double q_nb[4];               /**< Navigation data, computation frame to NED quaternion, L-, I-, J- and K- component */
    double pos_llh[3];            /**< Navigation data, latitude and longitude position in WGS84 in [rad], altitude position in [m] */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint8_t reserved[3];           /**< reserved for further use */
    uint32_t status_registers[32]; /**< see XCOMMSG_CANSTATUS2_IDX_* */
    XCOMFooter footer;
} XCOMmsg_CANSTATUS2;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    double latitude;                    /**< Latitude in [read] */
    double longitude;                   /**< Longitude in [rad] */
    float altitude;                     /**< Altitude in [m] */
    float position_stddev[3];           /**< Latitude, longitude and altitude standard deviation in [m] */
    uint8_t solution_valid;             /**< Position solution valid/invalid */
    uint8_t obs_used;                   /**< Observations used in position solution */
...
enum XCOMGreenlightAlarmCategory {
    XCOM_ALARM_OFF      = 0,    /**< TBD */
    XCOM_ALARM_WARNING  = 1,    /**< TBD */
    XCOM_ALARM_CRITICAL = 2,    /**< TBD */
    XCOM_ALARM_OK       = 3,    /**< TBD */
    XCOM_ALARM_ERROR    = 4,    /**< TBD */
};
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t payload[32];        /**< Version number of the host operating sys- tem. */
    XCOMFooter footer;
...
/**
 * This parameter is read-only and holds the git hash
...
/**
 * This parameter is read-only and holds the git branch name
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                             /**< XCOM header */
    XCOMParHeader param_header;                    /**< XCOM parameter header */
    int8_t pipeline_id[CIPIPELINEID_STRLENGTH];    /**< git branch name */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                              /**< XCOM header */
    XCOMParHeader param_header;                     /**< XCOM parameter header */
    uint8_t usb_config;                             /**< USB peripherie configuration */
    uint8_t reserved[3];                            /**< Reserved for further use */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;             /**< XCOM header */
    XCOMParHeader param_header;    /**< XCOM parameter header */
    float xyz[3];                  /**< Installation misalignment between the INS enclosure and the vehicle frame */
    XCOMFooter footer;
} XCOMParIMU_MISALIGN;
...
/**
 * This parameter adjusts the timestamp of the inertial data  which is used to synchronize IMU measurements with external sensors.
 * It can be used to correct for known and constant group or transmission delays.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
/**
 * This parameter defines calibration parameters which will be used by the system to enable additional correction of the
 * calibrated IMU values.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    float scf_acc[3];           /*<< Accelerometer scale factors */
    float bias_acc[3];          /*<< Accelerometer bias [m/s/s] */
    float scf_omg[3];           /*<< Gyro scale factors */
    float bias_omg[3];          /*<< Gyro bias [rad/s] */
    XCOMFooter footer;
} XCOMParIMU_CALIBCOARSE;

/**
 * This parameter stores crosscoupling matrices which are used by the system after application of PARIMU_CALIB and before strapdown
 * and EKF calculation. This parameter is used in the calibration procedure to correct for the misalignment between ISA and enclosure.
...
typedef struct XCOM_STRUCT_PACK {
     ...
     * Idx. 6-8: 3. Col
     */
    double cc_acc[9];            /**< Accelerometer crosscoupling matrix */
    double cc_omg[9];            /**< Gyro crosscoupling matrix */
    XCOMFooter footer;
...
/**
 * This parameter configures the downsampling behavior prior to strapdown calculations if the prescaler value in PARSYS_PRESCALER is
 * greater than 1.
 * #name: XCOMParIMU_COMPMETHOD
 */
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t method;            /**< Method used to downsample:0 = snapshot (module diabled); 1 = averaging 2 = coning and sculling */
    XCOMFooter footer;
} XCOMParIMU_COMPMETHOD;

/**
 * This parameter enables/disables the internal gyro bias model
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t enable;             /**< enable/disable bias model */
    uint8_t reserved[3];        /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParIMU_GYROBIASMODELING;

/**
 * This parameter enables/disables the calibration over temperature
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t enable;             /**< enable/disable calibration module */
    uint8_t reserved[3];        /**< reserved for further use */
    XCOMFooter footer;
} XCOMParIMU_CALIBTEMPERATURE;
...
/**
 * This parameter configures coordinates of the accelerometers in the body frame. These values are used to compensate for the size effect.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    double r_acc[9];            /**< [0-3]: Position of the x-axis accelerometer in [m]
                                     [4-6]: Position of the y-axis accelerometer in [m]
                                     [7-9]: Position of the z-axis accelerometer in [m] */
    XCOMFooter footer;
} XCOMParIMU_ACCLEVERARM;

/**
 * This parameter configures the strapdown algorithm for different types of IMUs.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                  /**< XCOM header */
    XCOMParHeader param_header;         /**< XCOM parameter header */
    uint8_t is_increment;               /**< Defines IMU measurement increments:
                                              0: angular and velocity increments
                                              1: angular rates and accelerations increments */
    uint8_t delta_v_frame;              /**< Defines the ∆V frame. If set to 0, the body frame of the previous sampling interval is used
                                             (∆V b(k-1)). Otherwise, it is assumed that ∆V is referenced in the new body frame (∆V b(k)) */
    uint8_t n_delta_theta;              /**< Number of ∆θ per velocity update interval. The attitude update may be computed at a higher
                                             rate than the velocity update */
    uint8_t use_secondary_accel_sensor; /**< Use secondary accelerometer instead of the primary one */
    XCOMFooter footer;
} XCOMParIMU_STRAPDOWNCONF;

/** This parameter configures the measurement range of the internal IMU */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    float range_accel;          /**< Accelerometer measurement range [m/s/s] */
    float range_gyro;           /**< Gyro measurement range [rad/s] */
    XCOMFooter footer;
} XCOMParIMU_RANGE;

/**
 * This parameter defines the rotation matrix from the iNAT standardv enclosure frame (z down, x axis pointing from the plugs into the
 * device) to a custom enclosure frame (e.g. z up, x axis pointing from the plugs into  the device). This matrix is applied after IMU
 * calibration and before using the data in strapdown calculations and all output logs.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    double c_enc_imu[9];        /**< Rotation matrix from iNAT standard en- closure frame to custom enclosure frame, column-wise */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint16_t password;          /**< Password protection */
    XCOMFooter footer;
} XCOMParGNSS_PORT;

/**
 * This parameter contains the path to the formware binary (used for GNSS firmware updates)
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t file_name[256];        /**< path to the update file */
    XCOMFooter footer;
...
/**
 * This parameter sets the GNSS aiding frame. The configured frame is valid for GNSS position and velocity aiding.
 * #name: XCOMParGNSS_AIDFRAME
 */
#define PARGNSS_AIDFRAME_SELECT_ECEF 0
#define PARGNSS_AIDFRAME_SELECT_LLH  1
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t power_mask;        /**< Antenna power selection mask */
    XCOMFooter footer;
...
/**
 * This parameter configures the GNSS threshold module
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    float elevation_cutoff;     /**< Elevation cutoff angle [rad] */
    uint8_t cn0_thresh_num_sv;  /**< Carrier to noise threshold */
    uint8_t cn0_thresh;         /**< Carrier to noise threshold */
    uint16_t reserved;          /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParGNSS_NAVCONFIG;

/**
 * This parameter scales/limits the standard deviation of the GNSS engine. For every payload entry of this parameter, only values ≥ 0 are accepted.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;              /**< XCOM header */
    XCOMParHeader param_header;     /**< XCOM parameter header */
    float stddev_scaling_pos;       /**< Scaling of the position standard deviation (default: 1.0) */
    float stddev_min_pos;           /**< Minimum of the permitted position standard deviation (default: 0.0) */
    float stddev_scaling_rtk;       /**< Scaling of the RTK position standard deviation (default: 1.0) */
    float stddev_min_rtk;           /**< Minimum of the permitted RTK position standard deviation (default: 0.0) */
    float stddev_scaling_vel;       /**< Scaling of the velocity standard deviation (default: 1.0) */
    float stddev_min_vel;           /**< Minimum of the permitted velocity standard deviation (default: 0.0) */
    XCOMFooter footer;
} XCOMParGNSS_STDDEV;
...
/**
 * This parameter configures the GNSS voter module
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t enable;             /**< Enables/Disables GNSS voter */
    uint8_t debug_enable;       /**< Enables/Disables debug output */
    uint16_t timeout;           /**< Timeout of the GNSS voter module in [s] */
    uint32_t voter_mode;        /**< GNSS voter mode */
    float hysteresis;           /**< GNSS voter hysteresis */
    uint8_t portINT;            /**< COM port of the internal GNSS engine */
    uint8_t portEXT;            /**< COM port of the external GNSS engine */
    uint16_t selection_mode;    /**< Selection mode */
    uint32_t baudINT;           /**< Baud rate of the internal GNSS engine */
    uint32_t baudEXT;           /**< Baud rate of the external GNSS engine */
    XCOMFooter footer;
} XCOMParGNSS_VOTER;
...
/**
 * This patameter contains the selected GNSS model
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
/**
 * This parameter is used to configure the various status mask fields in the RXSTATUSEVENT log
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    struct {
        uint32_t set_mask;
        uint32_t clear_mask;
    } config[5];                /**< Configuration mask */
    XCOMFooter footer;
} XCOMParGNSS_STATUSCONFIG;

/**
 * This parameter configures the RTK solution threshold
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t position_type;     /**< Position type threshold */
    XCOMFooter footer;
} XCOMParGNSS_RTKSOLTHR;

/**
 * This parameter configures the GNSS PPP feature
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t use_PPPPOS;         /**< Use PPP Solution in sensor fusion */
    uint8_t reserved[7];        /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParGNSS_TERRASTAR;

/**
 * This parameter configures the GNSS reference station feature. Field 1 of this parameter enables the Ntrip caster and server.
 * In order for changes to take effect, the configuration needs to be saved and the device needs to be rebooted.
 * #name: XCOMParGNSS_REFSTATION
 */
#define GNSS_REFSTATION_NTRIP_DISABLE       0    /**< Disable Ntrip caster */
#define GNSS_REFSTATION_NTRIP_ENABLE        1    /**< Enable Ntrip caster */
#define GNSS_REFSTATION_FIXPOS_DISABLE      0    /**< Position is not fixed in GNSS receiver */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint8_t baud_rate;            /**< 0: 9600; 1: 19200; 2: 38400; 3: 57600; 4: 115200; 5: 230400; 6: 460800 */
    XCOMFooter footer;
} XCOMParGNSS_REFSTATION;

/**
 * This parameter configures the fixed position of the GNSS receiver. It can be set either via this parameter or by successful completion of
 * the position averaging procedure, which can be triggered via PARGNSS_POSAVE.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;            /**< XCOM header */
    XCOMParHeader param_header;   /**< XCOM parameter header */
    double fixpos[3];             /**< Fix position: longitude [rad], latitude [rad] and altitude [m] */
    float fixpos_stdddev[3];      /**< Standard deviation of the entered fixed position [m] */
    XCOMFooter footer;
} XCOMParGNSS_FIXPOS;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    XCOMParHeader param_header;   /**< XCOM parameter header */
    double fixpos[3];             /**< Fix position: longitude [rad], latitude [rad] and altitude [m] */
    double pos_avg_max[3];        /**< Maximum of position averaging (read-only): longitude [rad], latitude [rad] and altitude [m] */
    double pos_avg_min[3];        /**< Minimum of position averaging (read-only): longitude [rad], latitude [rad] and altitude [m] */
    double fixpos_stdddev[3];     /**< Standard deviation of the entered fixed position [m] */
    XCOMFooter footer;
} XCOMParGNSS_FIXPOSV2;

/**
 * This parameter configures the PVT mode of the inernal GNSS engine.
 */
#define GNSS_PVTMODE_ROVER        0 /**< GNSS receiver is moving */
#define GNSS_PVTMODE_STATIC       1 /**< GNSS receiver is static */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;            /**< XCOM header */
    XCOMParHeader param_header;   /**< XCOM parameter header */
    uint8_t pvt_mode;             /**< If GNSS_PVTMODE_ROVER is selected, the receiver assumes that it is moving and it computes the best PVT.
                                       If GNSS_PVTMODE_STATIC is selected, the receiver assumes that it is static and it reports a constant position */
    uint8_t reserved[3];          /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParGNSS_PVTMODE;

/**
 * This parameter configures the position averaging on the integrated GNSS receiver.
 * #name: XCOMParGNSS_POSAVE
 */
#define GNSS_POSAVE_STATE_DISABLE      0 /**< Disable position averaging */
#define GNSS_POSAVE_STATE_ENABLE       1 /**< Enable position averaging */

#define GNSS_POSAVE_AVGSTATUS_OFF      POSAVE_AVGSTATUS_OFF         /**< Position averaging is turned off */
#define GNSS_POSAVE_AVGSTATUS_RUNNING  POSAVE_AVGSTATUS_RUNNING     /**< Position averaging is in progress and not yet finished */
#define GNSS_POSAVE_AVGSTATUS_COMPLETE POSAVE_AVGSTATUS_COMPLETE    /**< Position averaging process completed */

#define GNSS_POSAVE_SOLTYPE_NONRTK      0 /**< Use non RTK solution for position averaging */
#define GNSS_POSAVE_SOLTYPE_RTK         1 /**< Use RTK solution for position averaging */

typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;           /**< XCOM header */
    XCOMParHeader param_header;  /**< XCOM parameter header */
    float max_time;              /**< Maximum amount of time that positions are to be averaged [h] */
    float max_horizontal_stddev; /**< Desired horizontal standard deviation in [m] */
    float max_vertical_stddev;   /**< Desired vertical standard deviation in [m] */
    uint32_t averaging_status;   /**< Read-only status of the position averaging   process */
    uint32_t averaging_time;     /**< Elapsed time of averaging in [s] (read-only) */
    uint32_t averaging_samples;  /**< Number of samples in the average (read-only ) */
    uint8_t state;               /**< State of position averaging */
    uint8_t solution_type;       /**< Used solution type for position averaging */
    uint8_t reserved1[2];        /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParGNSS_POSAVE;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t features;          /**< Feature mask */
    uint32_t reserved[3];       /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParGNSS_FEATURES;

/**
 * This parameter configures the GNSS switcher module.
 * #name: XCOMParGNSS_SWITCHER
 */
#define PARGNSS_SWITCHER_INTERNAL_RECEIVER 0    /**< Use internal GNSS engine */
#define PARGNSS_SWITCHER_EXTERNAL_RECEIVER 1    /**< Use external GNSS engine */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t enable;             /**< Enables/Disables switcher modeule */
    uint8_t switcher;           /**< GNSS engine selection */
    uint16_t reserved1;         /**< Reserved for further use */
    XCOMFooter footer;
...
/**
 * #name: XCOMParGNSS_REFSTATIONV2
 */
typedef struct XCOM_STRUCT_PACK {
    uint16_t enable_rtcmv3_output : 1;
    uint16_t enable_rtcmv3_modem_output : 1;
    uint16_t enable_rtcm_output : 1;
    uint16_t enable_rtcm_modem_output : 1;
    uint16_t enable_rtca_uutput : 1;
    uint16_t enable_rtca_modem_output : 1;
    uint16_t enable_cmr_modem_output : 1;
    uint16_t enable_msm3_output : 1;
    uint16_t enable_msm4_output : 1;
    uint16_t enable_msm5_output : 1;
    uint16_t enable_msm6_output : 1;
    uint16_t enable_msm7_output : 1;
    uint16_t reserved : 4;
} XCOMRefstationOutputModes;

typedef struct XCOM_STRUCT_PACK {
    uint8_t enable_ntrip: 1;
    uint8_t enable_satel_modem: 1;
} XCOMRefStationSwitches;

typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                      /**< XCOM header */
    XCOMParHeader param_header;             /**< XCOM parameter header */
    XCOMRefStationSwitches switches;        /**< enable/disable Ntrip caster */
    uint8_t use_fixedpos;                   /**< use FIXPOS mode from GNSS engine */
    uint8_t baud_rate;                      /**< 0: 9600; 1: 19200; 2: 38400; 3: 57600; 4: 115200; 5: 230400; 6: 460800 */
    XCOMRefstationOutputModes output_mode;  /**< Reference station output mode */
    XCOMFooter footer;
} XCOMParGNSS_REFSTATIONV2;

/**
 * This parameter is used to switch between the receivers geoid model and the inat internal model.
 * #name: XCOMParGNSS_GEOIDSELECTION
 */
#define PARGNSS_GEOIDSELECTION_RECEIVER 0    /**< Use internal GNSS engine model */
#define PARGNSS_GEOIDSELECTION_INAT     1    /**< Use iNAT's internal geoid model */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t model_selection;    /**< Geoid model selection */
    uint8_t reserved[3];       /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParGNSS_GEOIDSELECTION;

/**
 * This parameter configures the GNSS antenna interface.
 * #name: XCOMParGNSS_ANTCONFIG
 */
#define PARGNSS_ANTCONFIG_SVCS       0x0001      /**< Enable antenna supply voltage control signal */
#define PARGNSS_ANTCONFIG_SCD        0x0002      /**< Enable short circuit detection */
#define PARGNSS_ANTCONFIG_OCD        0x0004      /**< Enable open circuit detection */
#define PARGNSS_ANTCONFIG_PDWNONSCD  0x0008      /**< Power down antenna supply if short circuit is detected. */
#define PARGNSS_ANTCONFIG_RECOVERY   0x0010      /**< Enable automatic recovery from short state */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint16_t config;            /**< antenna interface configuration */
    uint8_t reserved[2];       /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParGNSS_ANTCONFIG;
 ...
 * #name: XCOMParMAG_CAL
 */
#define PARMAG_CALIBRATION_DISABLE 0    /**< Disable magnerometer calibration */
#define PARMAG_CALIBRATION_ENABLE  1    /**< Enable magnetometer calibration */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    float cc[9];                /**< Calibration matrix */
    float b[3];                 /**< Bias vector */
    uint32_t calib_enable;      /**< Enables/Disables calibration */
    XCOMFooter footer;
} XCOMParMAG_CAL;

/**
 * This parameter holds the on-board magnetometer calibration procedure status of the 2D magnetometer calibration routine.
 * A more accurate 3D calibration routine is also available.
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    int32_t state;              /**< Magnetometer calibration status */
    XCOMFooter footer;
} XCOMParMAG_CALSTATE;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint8_t source_selection;   /**< NMEA0183 source selection */
    uint8_t reserved1;          /**< Reserved for further use */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t tx_mask[2];        /* Idx0: UART; Idx1: UDP */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t divisor[2];        /* Idx0: UART; Idx1: UDP */
    XCOMFooter footer;
 ...
 * value PARODO_TIMEOUT, a ZUPT will be performed (only if odometer is enabled via the activation mask of the PAREKF_ZUPT parameter.
 ...
 */
#define PARODO_MODE_DISABLE           0 /**< Disable odometer module */
#define PARODO_MODE_ENABLE            1 /**< Enable odometer module */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    struct {
        uint16_t enable;            /**< Odometer enable/disable switch */
        uint16_t mode;              /**< Odometer mode */
    } odometer[PARODO_MODE2_MAX_ODOMETERS];
    uint16_t deglitcher[2];     /**< Deglitcher value of channel A/B in [internal ticks], default: 10 */
    XCOMFooter footer;
} XCOMParODO_MODE2;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint16_t reserved2;         /**< Reserved for further use */
    float constraints_stddev;   /**< Standard deviation of the odometer constraints, default: 2.0 */
    XCOMFooter footer;
...
/**
 * This patameter configures the quadrature encoder module.
 * #name: XCOMParODO_EQEP
 */
#define PARODO_EQEP_MODE_QUADRATURE  (1 << 0) /**< 1: Quadrature mode; 0: Direction mode */
#define PARODO_EQEP_MODE_EXTCLOCKRAT (1 << 1) /**< 1: 1x resolution; 0: 2x resolution (direction mode */
#define PARODO_EQEP_MODE_SWAPINPUTS  (1 << 2) /**< 1: Clock inputs are swapped; 0: Clock inputs are not swapped*/
#define PARODO_EQEP_MODE_QAP         (1 << 3) /**< 1: Negates A input; 0: No effect */
#define PARODO_EQEP_MODE_QBP         (1 << 4) /**< 1: Negates B input; 0: No effect */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t mode;              /**< EQEP opertaion mode */
    XCOMFooter footer;
} XCOMParODO_EQEP;
...
/**
 * This patameter configures the baud rate of the ARINC825 module
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint16_t mode;              /**< CAN operation mode (see XCOM_PARCAN_DEVICECONFIG_MODE) */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    union {
        uint32_t value;
        ParCAN_SIMPLECAN_TxMaskType bits;
    } tx_mask;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t activation_mask;    /**< Bus termination activation mask (see PARCAN_BUSTERMINATION_CAN*) */
    uint8_t reserved[3];        /**< Reserved for further use */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                          /**< XCOM header */
    XCOMParHeader param_header;                 /**< XCOM parameter header */
    uint8_t suffix[PARREC_SUFFIX_BUFFERSIZE];   /**< User definied log file suffix */
    XCOMFooter footer;
} XCOMParREC_SUFFIX;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint64_t max_file_size;     /**< Max. file size in MB */
    XCOMFooter footer;
} XCOMParREC_FILESIZE;
 ...
 * obtained from the extended Kalman filter. If the heading standard deviation falls below ThrHDG (field 5) during static alignment,
 * alignment will automatically terminated and the INS is ready to move.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    float summated_acceleration_limit; /**< summated acceleration Limit in [m/s/2] */
    uint16_t delay;                    /**< Delay in [ms] */
    uint8_t automatic_zupt;            /**< Automatic ZUPT aiding */
    uint8_t reserved;                  /**< reserved for further use */
    uint32_t mask;                     /**< Activation mask */
    XCOMFooter footer;
} XCOMParEKF_ZUPT2;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint32_t mask;              /**< An activition mask for changing the 'mode' bitfield. Only these values are changed for which the mask is enabled. */
    XCOMFooter footer;
} XCOMParEKF_OUTLIER;
 ...
 * charts in DOC141126064. The parameter entries only accept values of a certain range, for a detailed description see the PAREKF_STARTUPV2 parameter
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
/**
 * This parameter defines the default heading. The default heading will be loaded, if the stored heading is invalid. The parameter entries only accept
 * values of a certain range, for a detailed description see the PAREKF_STARTUPV2 parameter
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    double thr_stddev_pos;      /**< Standard deviation threshold of the position aiding module. If the standard deviation is larger than
 ...
 * #name: XCOMParEKF_SCHULERMODE
 */
#define PAREKF_SCHULERMODE_DISABLE 0 /**< Disable schuler mode */
#define PAREKF_SCHULERMODE_ENABLE  1 /**< Enable schuler mode */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                      /**< XCOM header */
    XCOMParHeader param_header;             /**< XCOM parameter header */
    uint8_t reference_frame;                /**< Reference frame */
    uint8_t reserved[3];                    /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParEKF_REFFRAME;
...
/**
 * This parameter configures the automatic realignment
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                      /**< XCOM header */
    XCOMParHeader param_header;             /**< XCOM parameter header */
    uint8_t extpos_enable;                  /**< Enable/disable automatic realignment based on ext. position updates */
    uint8_t extpos_reserved[3];             /**< Reserved for further use */
    uint32_t extpos_max_outliers;           /**< Number of consecutive outliers (ext. position) */
    uint8_t reserved[32];                   /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParEKF_AUTOREALIGN;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    float time_thr;                         /**< Threshold for time test in [m] */
 ...
 * are performed to refine the estimates of attitude (and heading for device with gyro compassing capability). Af- ter a duration controlled by the
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t state_freeze_mask; /**< Bitmask for freezing states in realtime navigation */
    XCOMFooter footer;
...
/**
 * This parameter contains the realignment condition bit mask. Setting an entry to ’1’ en- ables a condition in the automatic realignment
 ...
 * #name: XCOMParEKF_ODOCHECK
 */
#define EKF_ODOCHECK_DISABLE    0    /**< Disable odometer outlier detection module */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    int32_t max_error_gnss;     /**< Allowed GNSS errors */
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    XComNisThrType psr_vel;     /**< Outlier configuration for GNSS velocity aiding */
    XComNisThrType psr_pos;     /**< Outlier configuration for GNSS position aiding */
    XComNisThrType rtk_pos;     /**< Outlier configuration for GNSS RTK position aiding */
    XCOMFooter footer;
} XCOMParEKF_NISTHR;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t enable_pos_stddev;
    float pos_stddev;
    uint32_t enable_hdg_stddev;
    float hdg_stddev;
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;              /**< XCOM header */
    XCOMParHeader param_header;    /**< XCOM parameter header */
    uint8_t reference_frame;       /**< Output reference frame */
    uint8_t reserved[3];           /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParDAT_REFFRAME;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t mode;               /**< Mode selection */
    uint8_t reserved_1;         /**< Reserved for further use */
    uint8_t interface;          /**< Reserved for further use */
...
/**
 * This parameter configures the iMAR netcheck module
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t enable;            /**< Enables/Disables iMAR netcheck */
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    struct {
        ...
        uint8_t trigger;                /**< XCOM trigger type */
        uint16_t reserved1;             /**< Reserved for further use */
    } log_list[XCOM_LOGS_PER_CHANNEL];
    XCOMFooter footer;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint8_t password[PARXCOM_NTRIP_STRINGLENGTH];      /**< Ntrip caster passeord */
...
/**
 * This parameter configures the internal recording module for calibration recording
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                  /**< XCOM header */
    union {
        ...
        struct {
            uint16_t param_id;
            uint8_t enable_inssol;      /**< Enables/Disables INSSOL message */
            uint8_t is_request;
        };
    };
    uint8_t enable;                     /**< Enables/Disables calibration processing */
    uint8_t channel;                    /**< XCOM channel selection */
    uint16_t divider;                   /**< Output divider */
    uint8_t path_name[256];                /**< Name of recording file */
    XCOMFooter footer;
} XCOMParXCOM_CALPROC;

/**
 * This parameter configures the port of the internal UDP server.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t port;              /**< UDP port */
    uint8_t hidden_mode;        /**< Hidden mode: if  true, the system will not respond to the UDP broadcasts */
    uint8_t reserved[3];        /**< reserved for further use */
    XCOMFooter footer;
} XCOMParXCOM_BROADCAST;
...
/**
 * This parameter configures the monitor output
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t log_level;         /**< Monitor log level */
    uint32_t activation_mask;   /**< Monitor activation mask */
    XCOMFooter footer;
} XCOMParXCOM_MONITOR;

/**
 * This parameter configures the debug agent
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t enable;            /**< Enables/Disables debug agent */
    XCOMFooter footer;
} XCOMParXCOM_QCONN;

/**
 * This parameter configures fast-boot-mode
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t enable;            /**< Enables/Disables fast boot */
    XCOMFooter footer;
} XCOMParXCOM_FASTBOOT;

/**
 * This parameter configures the debug console
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t enable;            /**< Enables/Disables debug console */
    XCOMFooter footer;
} XCOMParXCOM_CONSOLE;

/**
 * This parameter configures the internal nameserver.
 * #name: XCOMParXCOM_DNSCONFIG
 */
#define PARXCOM_DNSCONFIG_MAX_SERVERADRR     16         /**< Max. length of servers ip address */
#define PARXCOM_DNSCONFIG_MAX_DOMAIN        128         /**< Max. length of domain name */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                                  /**< XCOM header */
    XCOMParHeader param_header;                         /**< XCOM parameter header */
    int8_t server[PARXCOM_DNSCONFIG_MAX_SERVERADRR];    /**< Nameserver ip address (192.168.xxx.xxx) */
    int8_t domain[PARXCOM_DNSCONFIG_MAX_DOMAIN];        /**< Domain name */
    uint8_t enable;                                     /**< Enable nameserver */
    uint8_t reserved[3];                                /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParXCOM_DNSCONFIG;
...
/**
 * This parameter is used to configure items related to the XComMsgAbdProtocol application.
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t destination_port;  /**< XComMsgAbdProtocol destination port */
    uint32_t source_port;       /**< XComMsgAbdProtocol source port */
    XCOMFooter footer;
} XCOMParXCOM_ABDCONFIG;

/**
 * This parameter allows to set up pre-configured output frames (PARIMU_C_ENC_IMU) for different custom coordinate systems.
 * Depending on the value of this parameter, the output roll and pitch angles are also adjusted.
 * #name: XCOMParXCOM_FRAMEOUT
 */
#define PARXCOM_FRAMEOUT_DIN9300  0
#define PARXCOM_FRAMEOUT_DIN70000 1
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t output_frame;      /**< iNAT output frame (0: DIN9300; 1: DIN70000) */
    XCOMFooter footer;
...
/**
 * This parameter configures the HDLC interface
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t mode;               /**< HDLC mode */
    uint8_t clock;              /**< HDLC clock speed */
    uint8_t invert_data;        /**< Inverts the HDLC data */
    uint8_t invert_clock;       /**< Inverts the HDLC clock */
    uint16_t reserved;          /**< Reserved for further use */
...
/**
 * This parameter configures the FPGA timing register
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t timing_register;    /**< Content of the FPGA timing register */
    uint8_t reserved1;          /**< Reserved for further use */
    uint16_t user_timer[3];     /**< User timer values */
    uint16_t reserved2;         /**< Reserved for further use */
    uint16_t password;          /**< Password */
    XCOMFooter footer;
} XCOMParFPGA_TIMING;
...
/**
 * This parameter configures the FPGA timer matrix.
 * #name: XCOMParFPGA_TIMER
 */
#define FPGA_TIMERMATRIX_SIZE 32
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                      /**< XCOM header */
    XCOMParHeader param_header;             /**< XCOM parameter header */
    uint32_t timer[FPGA_TIMERMATRIX_SIZE];  /**< Timer matrix */
    uint16_t reserved1;                     /**< Reserved for further use */
    uint16_t password;                      /**< Password */
    XCOMFooter footer;
} XCOMParFPGA_TIMER;
...
/**
 * This parameter configures the FPGA interface matrix
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t interface[23];     /**< Interface matrix */
    uint16_t reserved1;         /**< Reserved for further use */
    uint16_t password;          /**< Password */
    XCOMFooter footer;
...
/**
 * This parameter configures the FPGA control register
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint16_t control_register;  /**< Content of the FPGA control register */
    uint16_t reserved1;         /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParFPGA_CONTROLREG;

/**
 * This parameter configures the FPGA trigger timeout register
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint16_t trigger_timeout;   /**< Trigger timeout value */
    uint16_t reserved1;         /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParFPGA_TRIGTIMEOUT;

/**
 * This parameter configures the power-up threshold
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    float threshold;            /**< Power-up threshold in [V] */
    float reserved1;            /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParFPGA_POWERUPTHR;

/**
 * This parameter configures the holdover time
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    float holdover_time;        /**< Holdover time in [sec] */
    XCOMFooter footer;
} XCOMParFPGA_HOLDOVERTIME;

/**
 * This parameter configures the VMS PBIT module
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                  /**< XCOM header */
    XCOMParHeader param_header;         /**< XCOM parameter header */
    uint8_t enable;                     /**< Enables/Disables  the VMS PBIT module */
    uint8_t odo_idx;                    /**< Odometer selection (0-2) */
    uint16_t reserved;                  /**< Reserved for further use */
    float max_runtime_sec;              /**< Max. test runtime in [sec] */
    float tolerance_percent;            /**< Test tolerance in [%] */
    int32_t nominal_ticks_per_second;   /**< Nominal ticks per second */
    int32_t standstill_limit_ticks;     /**< Standstill threshold in [ticks] */
    XCOMFooter footer;
} XCOMParFPGA_VMSPBIT;

/**
 * This parameter configures the IEP245 interface matrix
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;              /**< XCOM header */
    XCOMParHeader param_header;     /**< XCOM parameter header */
    uint8_t aInterfaceMatrix[32];   /**< Interface matrix */
    XCOMFooter footer;
} XCOMParFPGA_INTMAT245;

/**
 * This parameter is read-only and contains the FPGA type.
 * #name: XCOMParFPGA_TYPE
 */
...
#define XCOM_PARFPGA_TYPE_288V5     6
#define XCOM_PARFPGA_TYPE_M200OEMV4 7
#define XCOM_PARFPGA_TYPE_IPSS      8
#define XCOM_PARFPGA_TYPE_IANARO    9
#define XCOM_PARFPGA_TYPE_M300     10

typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t type;              /**< FPGA type */
    XCOMFooter footer;
} XCOMParFPGA_TYPE;

/**
 * This parameter contains the FPGA status word.
 * #name: XCOMmsg_STATFPGA
 */
typedef struct {
    uint32_t go : 1;
    uint32_t power_error : 1;
    uint32_t temp_error : 1;
    uint32_t imu_error : 1;
    uint32_t odometer_error : 1;
    uint32_t supervisor_error : 1;
    uint32_t timing_error : 1;
    uint32_t imu_power_error : 1;
    uint32_t fpga_mode_service : 1;
    uint32_t fpga_mode_safe : 1;
    uint32_t fpga_mode_init : 1;
    uint32_t fpga_mode_operational : 1;
    uint32_t fpga_mode_standby : 1;
    uint32_t fpga_mode_update : 1;
    uint32_t ppt : 1;
    uint32_t power_loss : 1;
    uint32_t ppd_0 : 1;
    uint32_t ppd_1 : 1;
    uint32_t ppd_2 : 1;
    uint32_t trigger_timeout : 1;
    uint32_t pwr_good_3v3 : 1;
    uint32_t pwr_good_5v_out : 1;
    uint32_t pwr_good_5v : 1;
    uint32_t capgd : 1;
    uint32_t reserved : 8;
} XCOMFpgaStatus;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;              /**< XCOM header */
    XCOMParHeader param_header;     /**< XCOM parameter header */
    uint32_t power_status_min;      /**< FPGA power status register */
    uint32_t power_status_max;      /**< FPGA power status register */
    uint16_t fpga_status;           /**< FPGA status register */
    uint16_t supervisor_status;     /**< FPGA supervisor register */
    uint8_t imu_status;             /**< FPGA IMU status register */
    uint8_t temperature_status;     /**< FPGA temperature register */
    uint16_t fpga_status_ext;       /**< Extended FPGA status register */
    XCOMFooter footer;
} XCOMmsg_STATFPGA;

/**
 * This parameter configures the IEP245 global configuration register
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                  /**< XCOM header */
    union {
        ...
        struct {
            uint16_t param_id;
            uint8_t ppt_divider;        /**< PPT divider */
            uint8_t is_request;
        };
    };
    uint32_t config;                    /**< Global configuration register */
    XCOMFooter footer;
} XCOMParFPGA_GLOBALCONF;

/**
 * This parameter configures the HDLC pin configuration
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t pin_mode;          /**< HDLC pin mode */
    XCOMFooter footer;
} XCOMParFPGA_HDLCPINMODE;

/**
 * This parameter configures the FPGA power register.
 * #name: XCOMParFPGA_POWER
 */
/* FPGA Power Switches */
...
#define XCOM_FPGAPOWER_IANARO_24VEXT        (1 << 6)
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t power_switches;    /**< FPGA power switches */
    XCOMFooter footer;
} XCOMParFPGA_POWER;

/**
 * This patameter configures the FPGA alarm threshold
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint32_t alarm_thr;         /**< Alarm threshold */
    XCOMFooter footer;
} XCOMParFPGA_ALARMTHR;

/**
 * This parameter configures the PPT output
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint16_t ppt_divider;       /**< PPT putput divider */
    uint8_t ppt_pulse_width;    /**< PPT pulse width */
    uint8_t reserved1;          /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParFPGA_PPTCONFIG;

/**
 * This parameter configures the automatic power-on feature
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t enable_auto_wakeup; /**< Enables/Disables automatic power-on */
    uint8_t interval;           /**< Retry interval in [sec] */
    uint8_t retries;            /**< Max. number of retries */
    uint8_t reserved1;          /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParFPGA_AUTOWAKEUP;

/**
 * This parameter reads/writes the registers of the MCP23S08 device
 */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;          /**< XCOM header */
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t gpio_reg;           /**< GPIO register */
    uint8_t reserved_1[3];      /**< Reserved for further use */
    XCOMFooter footer;
} XCOMParFPGA_MCP23S08;

/**
 * This parameter configures the CSAC module.
 * #name: XCOMParFPGA_CSAC
 */
...
typedef struct XCOM_STRUCT_PACK {
    ...
    XCOMParHeader param_header; /**< XCOM parameter header */
    uint8_t mode;
    uint32_t pps_discipline_time_constant;
    uint32_t pps_discipline_cable_length_comp;
    uint32_t pps_phase_thr;
    XCOMFooter footer;
} XCOMParFPGA_CSAC;
...
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;
    XCOMParHeader param_header;
    double timing_param_h[3];
    uint32_t timeout_int_clock;
    uint32_t timeout_ext_clock;
    XCOMFooter footer;
} XCOMParFPGA_CSACAUTO;
 ...
 *
 */
#define PARXCOM_HW245_EVENIN_ENABLE (1 << 0) /**< SYNC_1 */
#define PARXCOM_HW245_PPT_ENABLE    (1 << 1) /**< SYNC_2 */
#define PARXCOM_HW245_TOV_ENABLE    (1 << 2) /**< SYNC_3 */
#define PARXCOM_HW245_PPS_ENABLE    (1 << 3) /**< SYNC_4 */
#define PARXCOM_HW245_EVENIN_INVERT (1 << 4) /**< INVERT --> FALLING EDGE */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;
    union {
        XCOMParHeader param_header;
...
typedef struct XCOM_STRUCT_PACK {
    ...
    uint32_t period;
    uint32_t timer;
} XCOMAring429ListItem;

typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;
    XCOMParHeader param_header;
    XCOMAring429ListItem list[32];
    XCOMFooter footer;
} XCOMParARINC429_LIST;

/**
 * Parameters for Project: KEP-I000109_iATTHEMO-TRIDENT_Skywave.
 * This parameter controls the behavior of servo motor controller and system.
 * #name: XCOMParIATTHEMO_PARAM
 */
#define XCOMPAR_PLUGINID_IATTHEMO XCOM_PLUGINID_ATTHEMO   /**< Plugin ID of IATTHEMO Parameter message */
typedef struct XCOM_STRUCT_PACK{
    float       rotation_velocity_limit;    /** max rotation velocity [rad/s] */
    float       acceleration_limit;         /** max motor acceleration [rad/s*s] */
    float       angle_tolerance;            /** target angle accuracy [rad] */
    float       min_angle;                  /** minimum angle [rad] */
    float       max_angle;                  /** maximum angle [rad] */
    float       move_to_pos;                /** In Mode 4 can be moved to a specified position[rad]*/
    uint32_t    rotation_interval;          /** time between next rotation*/
    uint8_t     move;                       /** flag to turn rotation on [0,1] */
    uint8_t     auto_mode;                  /** mode to control rotation. STOP=0, Rotation=1, Alignment=2, Single Shot=3*/
    uint8_t     save_to_config;             /** flag to save parameters in configuration file of plugin */
    uint8_t     reserved;                   /** reserved for further use*/
}XCOMParIATTHEMO_PARAM_payload;

typedef struct XCOM_STRUCT_PACK{
    XCOMHeader header;
    XCOMParHeader param_header;
    XCOMPluginHeader plugin_header;
    XCOMParIATTHEMO_PARAM_payload payload;
    XCOMFooter footer;
}XCOMParIATTHEMO_PARAM;

/**
 * ARINC429 loopback data.
 * #name: XCOMmsg_ARINC429LOOPBACK
 */
#define XCOMMSG_PLUGINID_ARINC429 XCOM_PLUGINID_ARINC429  /**< Plugin ID of heave message */
typedef struct XCOM_STRUCT_PACK {
    double gps_time;
    uint16_t gps_week;
    uint16_t channel;
    uint32_t arinc429_labeldata;
} XCOMArinc429LabelData;

#define XCOMMSG_ARINC429PLUGIN_CHANNELS             4   /**< Number of supported channels */
#define XCOMMSG_ARINC429PLUGIN_LABELS_PER_CHANNEL   35  /**< Number of supported labels per channel */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;
    XCOMPluginDataHeader plugin_header;
    XCOMArinc429LabelData data[XCOMMSG_ARINC429PLUGIN_CHANNELS*XCOMMSG_ARINC429PLUGIN_LABELS_PER_CHANNEL];
    XCOMFooter footer;
} XCOMmsg_ARINC429LOOPBACK;

/**
 * This parameter contains the ARINC429 bus configuration for PCB-I000103.
 * #name: XCOMParARINC429_BUSCONFIG
 */
 enum ARINC429DataID {
     ARINC429DataID_Invalid,                    /**<  */
     ARINC429DataID_InsGnssAltitude,            /**< Aircraft’s height above WGS84 ellipsoid (integrated solution of the internal Kalman filter) */
     ARINC429DataID_InsGnssLatitude,            /**< Aircraft’s latitude (integrated solution of the internal Kalman filter) */
     ARINC429DataID_InsGnssLongitude,           /**< Aircraft’s longitude (integrated solution of the internal Kalman filter) */
     ARINC429DataID_InsGnssLatitudeFine,        /**<  */
     ARINC429DataID_InsGnssLongitudeFine,       /**<  */
     ARINC429DataID_UtcFine,                    /**<  */
     ARINC429DataID_UtcTime,                    /**<  */
     ARINC429DataID_BaroAltitude,               /**<  */
     ARINC429DataID_TrueAirspeed,               /**<  */
     ARINC429DataID_AngleOfAttack,              /**<  */
     ARINC429DataID_SideSlipAngle,              /**<  */
     ARINC429DataID_UtcDate,                    /**<  */
     ARINC429DataID_InsStatus,                  /**<  */
     ARINC429DataID_AlignStatus,                /**<  */
     ARINC429DataID_SpeedOverGround,            /**< Aircraft’s speed over ground (integrated solution of the internal Kalman filter) */
     ARINC429DataID_TrueTrackAngle,             /**<  */
     ARINC429DataID_Yaw,                        /**< Aircraft’s yaw angle */
     ARINC429DataID_YawMagnetic,                /**<  */
     ARINC429DataID_Pitch,                      /**< Aircraft’s pitch angle */
     ARINC429DataID_Roll,                       /**< Aircraft’s roll angle */
     ARINC429DataID_BodyAccelLongitudinal,      /**<  */
     ARINC429DataID_BodyAccelLateral,           /**<  */
     ARINC429DataID_BodyAccelNormal,            /**<  */
     ARINC429DataID_TrackAngleRate,             /**<  */
     ARINC429DataID_InertialPitchRate,          /**< Aircraft’s body pitch rate */
     ARINC429DataID_InertialRollRate,           /**< Aircraft’s body roll rate */
     ARINC429DataID_InertialYawRate,            /**< Aircraft’s body yaw rate */
     ARINC429DataID_VerticalVelocity,           /**< Estimated down velocity in WGS84 reference frame (up direction: positive) */
     ARINC429DataID_NorthSouthVelocity,         /**< Estimated north velocity in WGS84 reference frame (north direction: positive) */
     ARINC429DataID_EastWestVelocity,           /**< Estimated east velocity in WGS84 reference frame (east direction: positive) */
     ARINC429DataID_GnssHeight,                 /**<  */
     ARINC429DataID_Discrete2,                  /**<  */
     ARINC429DataID_HDop,                       /**<  */
     ARINC429DataID_VDop,                       /**<  */
     ARINC429DataID_DistanceToGo,               /**<  */
     ARINC429DataID_TimeToGo                    /**<  */
 };

typedef struct XCOM_STRUCT_PACK {
    uint8_t label_id;           /**< ARINC429 label ID */
    uint8_t data_id;            /**< ARINC429 data id (the data id defines the content of the label */
    uint8_t field_width;        /**< ARINC429 field width */
    uint8_t enable;             /**< Enable/Disable this label */
    uint32_t offset;            /**< ARINC429 label offset */
    uint32_t divider;           /**< ARINC429 output divider */
    double scale;               /**< Label scaling */
    double range;               /**< Label range */
} XCOMParARINC429_LabelConfigType;

typedef struct XCOM_STRUCT_PACK {
    XCOMParARINC429_LabelConfigType labels[XCOMMSG_ARINC429PLUGIN_LABELS_PER_CHANNEL];
    uint8_t high_speed;
    uint8_t rx_enable;
    uint8_t loopback_enable;
    uint8_t reserved;
} XCOMParARINC429ChannelConfigType;

typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;
    XCOMParHeader param_header;
    XCOMPluginHeader plugin_header;
    XCOMParARINC429ChannelConfigType channels[XCOMMSG_ARINC429PLUGIN_CHANNELS];
    XCOMFooter footer;
} XCOMParARINC429_BUSCONFIG;
...
typedef struct XCOM_STRUCT_PACK{
    ...
    float enc_pos_angle;                    /**< Current encoder position as angle in degree */
...
/**
 * TEC controller command package.
 * #name: XCOMmsg_TECCMD
 */
#define XCOMMSG_PLUGINID_TECCMD XCOM_PLUGINID_TECCMD                    /**< Plugin ID of TEC data message */
#define XCOMMSG_PLUGIN_TECCMD_NUMBER_OF_PWMS         2                  /**< Number of available pwm channels */

typedef struct {
    uint32_t processing_error : 1;                                      /**< Processing error within the control loop */
    uint32_t reserved : 31;                                             /**< Reserved for further use */
} XCOMTecCmdControlStatusBits;

typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                                                  /**< XCOM header */
    XCOMPluginDataHeader plugin_header;                                 /**< Plugin header: ID has to set to XCOMMSG_PLUGINID_TECCMD */
    union {
        XCOMTecCmdControlStatusBits bits;                               /**< Control loop error status */
        uint32_t value;
    } control_status;
    XCOMFpgaStatusType inat_status;                                     /**< iNAT error status */
    int32_t pwm_output[XCOMMSG_PLUGIN_TECCMD_NUMBER_OF_PWMS];           /**< PWM output in [%] */
    uint8_t reserved[8];                                                /**< Reserved for further use */
    XCOMFooter footer;
} XCOMmsg_TECCMD;

/**
 * Parameter for TEMPSTAB operation.
 * #name: XCOMParTEMPSTAB_PARAM
 */
#define XCOMPAR_PLUGINID_TEMPSTAB XCOM_PLUGINID_TEMPSTAB
typedef struct XCOM_STRUCT_PACK{
    int8_t  uart_no;                                                     /**< iNAT serial port used to communicate with the TEC */
    float   temp_target;                                                 /**< target temperature for controller */
    float   pid[3];                                                      /**< PID values for PID controller */
    uint8_t mode;                                                        /** 0 = unchanged 1 = automatic 2 = manual 3 = passive */
} XCOMParTEMPSTAB_PARAM_payload;

typedef struct XCOM_STRUCT_PACK{
    XCOMHeader                      header;                             /**< XCOM header */
    XCOMParHeader                   param_header;                       /**< XCOM parameter header */
    XCOMPluginHeader                plugin_header;                      /**< Plugin header: ID has to set to XCOMPAR_PLUGINID_TEMPSTAB */
    XCOMParTEMPSTAB_PARAM_payload   payload;                            /**< TEMPSTAB parameter see XCOMParTEMPSTAB_PARAM_payload */
    XCOMFooter                      footer;                             /**< XCOM footer */
} XCOMParTEMPSTAB_PARAM;

/**
 * RiCS to iAT, Communication mode C , Frequency: 100Hz
 * This message contains the iAT-input received from RiCS. It the current actuator state and status and user input from the HMI.
 * Moreover, it allows to send one timestamped value per message for the measurements.
 * If there are multiple measurements they are included one by one in successive messages.
 * A single valid timestamped value must be sent only once and not more than once in successive messages.
 * #name: XCOMmsg_IATIN
 */
#define XCOMMSG_PLUGINID_IATIN XCOM_PLUGINID_IATIN              /**< Plugin ID of IATIN data message */
#define XCOMPLUGIN_IAT_NUMBER_OF_FLAPS          4               /**< Number of supported flaps */
#define XCOMPLUGIN_IAT_SIZE_VSURGE_LUT          4               /**< Number of data points in vSurge look-up-table */
#define XCOMMSG_IATIN_MEASID_NONE               0               /**< Indicates that no measurement is available for this message */
#define XCOMMSG_IATIN_MEASID_STWSURGE           1               /**< Indicates that the measurement being sent is StwSurge */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                                          /**< XCOM header */
    XCOMPluginDataHeader plugin_header;                         /**< XCOM plugin header */
    uint32_t meas_id;                                           /**< ID of measurement */
    double meas_gps_time;                                       /**< GPS timestamp of measurement in [s] */
    double meas_value;                                          /**< Measurement value */
    uint8_t act_ready;                                          /**< Actuator ready flag */
    uint8_t reserved[3];                                        /**< Reserved for further use */
    float act_angle_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];       /**< Measured angle of main actuators in [rad] */
    float act_angle_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];        /**< Measured angle of aux actuators in [rad] */
    float act_offset_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];      /**< Desired angular offset for main actuators in [rad] */
    float act_offset_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];       /**< Desired angular offset for aux actuators in [rad] */
    float mute_gain;                                            /**< Manual gain for general reduction of the damping activity (0..1) */
    float roll_pitch_prio;                                      /**< Manual priority gain between pitch and roll damping (-1..1) */
    float reserved2;                                            /**< Reserved for further use */
    XCOMFooter footer;
} XCOMmsg_IATIN;

/**
 * iAT to RiCS, Communication mode B, Frequency: 100Hz
 * This message contains all the RiCS-specific iAT-output data to be sent to RiCS.
 * ActAngMain and ActAngAux are the control commands for the actuator angles computed by the motion damping algorithm.
 * The commanded angles are calculated for all the actuators that are marked as ready via field 7 of message IATIN.
 * Each actuator can be activated and deactivated individually without impacting the other activated actuators.
 * #name: XCOMmsg_IATOUT
 */
#define XCOMMSG_PLUGINID_IATOUT XCOM_PLUGINID_IATOUT                    /**< Plugin ID of IATOUT data message */
#define XCOMMSG_IATOUT_NUMBER_OF_FLAPS XCOMMSG_IATIN_NUMBER_OF_FLAPS    /**< Number of supported flaps */
typedef struct XCOM_STRUCT_PACK {
    XCOMHeader header;                                      /**< XCOM header */
    XCOMPluginDataHeader plugin_header;                     /**< XCOM plugin header */
    uint8_t cmd_acknowledge;                                /**< One enable acknowledge flag per actuator */
    uint8_t reserved[3];                                    /**< Reserved for further use */
    float cmd_angle_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];   /**< Commanded angle for main actuators in [rad] */
    float cmd_angle_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];    /**< Commanded angle for aux actuators in [rad] */
    float roll_rms;                                         /**< RMS of roll angle in [rad] */
    float roll_amp;                                         /**< Roll motion amplitude in [rad] */
    float roll_t;                                           /**< Roll motion time period in [s] */
    float pitch_rms;                                        /**< RMS of pitch angle in [rad] */
    float pitch_amp;                                        /**< Pitch motion amplitude in [rad] */
    float pitch_t;                                          /**< Pitch motion time period in [rad] */
    float vSurge;                                           /**< Surge velocity used in control algorithm (in m/s) */
    float reserved2[3];                                     /**< Reserved for further use */
    uint32_t iat_status;                                    /**< iAT error flags */
    XCOMFooter footer;
} XCOMmsg_IATOUT;

/**
 * This parameter contains the angular limits of the actuators.
 * Changed on or requested from iAT by RiCS, Communication mode A; Frequency: Once if required.
 * #name: XCOMParIAT_ANGLIM
 */
#define XCOMPAR_PLUGINID_IATANGLIM XCOM_PLUGINID_IATANGLIM
typedef struct XCOM_STRUCT_PACK{
    float ang_lim_low_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Lower angular limit for main actuator in [rad] */
    float ang_lim_up_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];      /**< Upper angular limit for main actuator in [rad] */
    float ang_lim_low_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];      /**< Lower angular limit for aux actuator in [rad] */
    float ang_lim_up_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];       /**< Upper angular limit for aux actuator in [rad] */
} XCOMParIAT_ANGLIM_payload;

typedef struct XCOM_STRUCT_PACK{
    XCOMHeader                      header;                     /**< XCOM header */
    XCOMParHeader                   param_header;               /**< XCOM parameter header */
    XCOMPluginHeader                plugin_header;              /**< Plugin header: ID has to set to XCOM_PLUGINID_IATANGLIM */
    XCOMParIAT_ANGLIM_payload       payload;
    XCOMFooter                      footer;                     /**< XCOM footer */
} XCOMParIAT_ANGLIM;

/**
 * This parameter contains the rate and acceleration limits of the actuators.
 * Changed on or requested from iAT by RiCS, Communication mode A; Frequency: Once if required.
 * #name: XCOMParIAT_RATEACCLIM
 */
#define XCOMPAR_PLUGINID_IATRATEACCLIM XCOM_PLUGINID_IATRATEACCLIM
typedef struct XCOM_STRUCT_PACK{
    float rate_lim_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Angular rate limit for main actuators in [rad/s] */
    float rate_lim_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Angular rate limit for aux actuators in [rad/s] */
    float acc_lim_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Acceleration limit for main actuators in [rad/s²] */
    float acc_lim_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Acceleration limit for aux actuators in [rad/s²] */
    float acc_stop_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Acceleration limit for stopping after deactivation for main actuators in [rad/s²] */
    float acc_stop_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Acceleration limit for stopping after deactivation for aux actuators in [rad/s²] */
} XCOMParIAT_RATEACCLIM_payload;

typedef struct XCOM_STRUCT_PACK{
    XCOMHeader                      header;                     /**< XCOM header */
    XCOMParHeader                   param_header;               /**< XCOM parameter header */
    XCOMPluginHeader                plugin_header;              /**< Plugin header: ID has to set to XCOM_PLUGINID_IATRATEACCLIM */
    XCOMParIAT_RATEACCLIM_payload   payload;
    XCOMFooter                      footer;                     /**< XCOM footer */
} XCOMParIAT_RATEACCLIM;

/**
 * This parameter contains the lever arms of the actuators (actuator position on the vessel).
 * Changed on or requested from iAT by RiCS, Communication mode A; Frequency: Once if required.
 * #name: XCOMParIAT_LEVARM
 */
#define XCOMPAR_PLUGINID_IATLEVARM XCOM_PLUGINID_IATLEVARM
typedef struct XCOM_STRUCT_PACK{
    float lev_arm_x_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Leverarm x-component (forward) in vessel body frame (in m) for the main actuators */
    float lev_arm_x_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Leverarm x-component (forward) in vessel body frame (in m) for the aux actuators */
    float lev_arm_y_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Leverarm y-component (forward) in vessel body frame (in m) for the main actuators */
    float lev_arm_y_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Leverarm y-component (forward) in vessel body frame (in m) for the aux actuators */
} XCOMParIAT_LEVARM_payload;

typedef struct XCOM_STRUCT_PACK{
    XCOMHeader                      header;                     /**< XCOM header */
    XCOMParHeader                   param_header;               /**< XCOM parameter header */
    XCOMPluginHeader                plugin_header;              /**< Plugin header: ID has to set to XCOM_PLUGINID_IATLEVARM */
    XCOMParIAT_LEVARM_payload       payload;
    XCOMFooter                      footer;                     /**< XCOM footer */
} XCOMParIAT_LEVARM;
 ...
 * This parameter contains the lever arms of the actuators (actuator position on the vessel).
 * Changed on or requested from iAT by RiCS, Communication mode A; Frequency: Once if required.
 * #name: XCOMParIAT_ALGO
 */
#define XCOMPAR_PLUGINID_IATALGO XCOM_PLUGINID_IATALGO
typedef struct XCOM_STRUCT_PACK{
    float t_sample; /**< Sampling time of algorithm (in s) */
    float in_filt_bw; /**< Bandwidth of input low pass (in Hz) */
    float out_filt_bw; /**< Bandwidth of output low pass (in Hz) */
    float rms_wdw_size; /**< Window size for RMS computation (in s) */
    float fb_gain_max_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Maximum feedback gain of stabilization (in s) for the main actuators */
    float fb_gain_max_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Maximum feedback gain of stabilization (in s) for the aux actuators */
    float fb_gain_rise_time_main[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Minimum time for feedback gain to reach its maximum (in s) for the main actuators */
    float fb_gain_rise_time_aux[XCOMPLUGIN_IAT_NUMBER_OF_FLAPS];     /**< Minimum time for feedback gain to reach its maximum (in s) for the aux actuators */
    float v_surge_fcn_in_pts[XCOMPLUGIN_IAT_SIZE_VSURGE_LUT];     /**< Data points for vSurge look-up-table, in-components (in m/s) */
    float v_surge_fcn_out_pts[XCOMPLUGIN_IAT_SIZE_VSURGE_LUT];     /**< Data points for vSurge look-up-table, out-components (in m/s) */
} XCOMParIAT_ALGO_payload;

typedef struct XCOM_STRUCT_PACK{
    XCOMHeader                      header;                     /**< XCOM header */
    XCOMParHeader                   param_header;               /**< XCOM parameter header */
    XCOMPluginHeader                plugin_header;              /**< Plugin header: ID has to set to XCOM_PLUGINID_IATALGO */
    XCOMParIAT_ALGO_payload         payload;
    XCOMFooter                      footer;                     /**< XCOM footer */
} XCOMParIAT_ALGO;
 ...
 * Aliases
 */
typedef XCOMmsg_STATFPGA XCOMParFPGA_STATUS;
