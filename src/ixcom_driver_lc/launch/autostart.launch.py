...
def generate_launch_description():
	...
	# Event handler that gets called, when the ixcom_driver_lc_node gets transitioned into the inactive state
	...
	# Event handler that gets called, when the ixcom_driver_lc_node gets transitioned into the active state
	...
	# 2. The ixcom_driver_lc_configure_event is triggered automatically, because it's part of the LaunchDescription.
	#    This kicks of the automated state transitioning process of the ixcom_driver_lc_node
	# 3. After the node transitions into the configured state, the event handler "ixcom_driver_configure_event" is triggered,
	#    which is part of this launch description. It internally logs the current state and emits the 
	#    "ixcom_driver_lc_activate_event", because its part of the event handlers entity list (which contains the actions that
	#    are executed, after the event took place). This start the state transition into the active state
	# 4. After the node has transitioned into the active state, the ixcom_driver_lc_active_state_handler is called,
	#    which logs the successful transition to the active state to the console
	
	return LaunchDescription([
		# Declare the launch arguments
		DeclareLaunchArgument("namespace", default_value="", description="Top-level namespace"),
		DeclareLaunchArgument("params_file", default_value=get_package_share_directory("ixcom_driver_lc") + "/params/default.yml", description="Path to a parameter file that will be passed to the ixcom_driver_lc node"),
		# Add the ixcom_driver_lc_node, as well as the necessary event handlers for automated lifecycle state transitioning
