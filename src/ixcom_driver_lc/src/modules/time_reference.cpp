...
TimeReference::TimeReference(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
    ...
    qos_(qos),
    tf2_(tf2) {
    std::thread th(&TimeReference::init, this);
    th.detach();
}
...
void TimeReference::handle_response(XCOMResp response) noexcept {
    if(response == XCOMResp::OK) {
        ...
        if(!init_done_) {
            ...
            timeref_msg_.header.frame_id = "inat_output_frame";
            ...
            msg_SYSSTAT_frq_ = calculateRateForDivider(div1, maintiming_, prescaler_);
            auto cmd_add_sysstat = xcom_.get_xcomcmd_enablelog(XCOM_MSGID_SYSSTAT, XComLogTrigger::XCOM_CMDLOG_TRIG_SYNC, div1);
            xcom_.send_message(cmd_add_sysstat);
            ...
            pub_ = node_->create_publisher<TimeReferenceMsg>(topic_name_, qos_);
            success_ = true;
            // cv_.notify_one();
        }
        init_done_ = true;
...
void TimeReference::activate() {
    ...
    std::thread th_mon(&TimeReference::frq_mon, this);
    th_mon.detach();
}
...
void TimeReference::updateSYSSTAT(const XCOMmsg_SYSSTAT &msg)
{

    gps_time_ = UpdateGPSTime(msg.header, leap_seconds_);
    if(timestamp_mode_ == Config::TimestampMode::GPS) {
        timeref_msg_.time_ref = gps_time_;
        timeref_msg_.source = "GPS";
    } else {
        timeref_msg_.time_ref = node_->now();
        timeref_msg_.source = "ROS";
    }

    publish();
}

void TimeReference::publish()
{
    timeref_msg_.header.stamp = (timestamp_mode_ == Config::TimestampMode::GPS) ? gps_time_ : node_->now();
    if(active_) {
        pub_->publish(timeref_msg_);
...
void TimeReference::frq_mon() {

    // bool unknown_state_printed = false;
    bool msg_SYSSTAT_ok = true;
    ...
    while(run_frq_mon_) {
        ...
        if(msg_SYSSTAT_ok && (msg_SYSSTAT_age_ > xcom_age_max_)) {
            msg_SYSSTAT_ok = false;
            RCLCPP_ERROR(node_->get_logger(), "[%s] %s (%s)", topic_name_.c_str(), "missing iNAT data", "XCOMmsg_SYSSTAT");
        } else if(!msg_SYSSTAT_ok && !(msg_SYSSTAT_age_ > xcom_age_max_)) {
            msg_SYSSTAT_ok = true;
            RCLCPP_INFO(node_->get_logger(), "[%s] %s (%s)", topic_name_.c_str(), "receiving iNAT data", "XCOMmsg_SYSSTAT");
        }

        if(msg_SYSSTAT_c_ > (mon_frq_ / msg_SYSSTAT_frq_)) {
            if(msg_SYSSTAT_age_ == age_max_val_) {
                msg_SYSSTAT_age_ = xcom_age_max_ + 1;
            } else {
                msg_SYSSTAT_age_++;
            }
            msg_SYSSTAT_c_ = 0;
