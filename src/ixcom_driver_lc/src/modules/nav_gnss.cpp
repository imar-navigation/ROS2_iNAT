...
NavGNSS::NavGNSS(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
    ...
    qos_(qos),
    tf2_(tf2) {
    std::thread th(&NavGNSS::init, this);
    th.detach();
}
...
void NavGNSS::handle_response(XCOMResp response) noexcept {
    if(response == XCOMResp::OK) {
        ...
        if(!init_done_) {
            ...
            navsatfix_msg_.header.frame_id = "primary_antenna";
...
void NavGNSS::handle_xcom_msg(const XCOMmsg_GNSSSOL &msg) noexcept {
    msg_GNSSSOL_age_ = 0;
    updateGNSSSOL(msg);
}

void NavGNSS::handle_xcom_param(const XCOMParGNSS_LOCKOUTSYSTEM& param) {
    par_GNSS_LOCKOUTSYSTEM_age_ = 0;
    setParData(param);
}

void NavGNSS::activate() {
    active_ = true;
    ...
    std::thread th_mon(&NavGNSS::frq_mon, this);
    th_mon.detach();
}

uint16_t NavGNSS::getSetupFreq() {
    return setup_freq_;
}

bool NavGNSS::connected() {
    return connected_;
}

bool NavGNSS::success() {
    return success_;
}

void NavGNSS::setParData(const XCOMParGNSS_LOCKOUTSYSTEM& param) {
...
void NavGNSS::updateGNSSSOL(const XCOMmsg_GNSSSOL &msg) {
        ...
        navsatfix_msg_.latitude = msg.lat / M_PI * 180.0F;
        navsatfix_msg_.longitude = msg.lon / M_PI * 180.0F;
        navsatfix_msg_.altitude = msg.alt + msg.undulation;

        navsatfix_msg_.position_covariance[0] = msg.stddev_pos[1];
        navsatfix_msg_.position_covariance[4] = msg.stddev_pos[0];
        navsatfix_msg_.position_covariance[8] = msg.stddev_pos[2];
...
void NavGNSS::publish() {

    if(!(parDataIsSet_ && gnssSolDataIsSet_)) {
        return;
    }
...
void NavGNSS::frq_mon() {

    bool unknown_state_printed = false;
    bool par_GNSS_LOCKOUTSYSTEM_ok = true;
    ...
    while(run_frq_mon_) {
        ...
        if(par_GNSS_LOCKOUTSYSTEM_ok && (par_GNSS_LOCKOUTSYSTEM_age_ > xcom_age_max_)) {
            par_GNSS_LOCKOUTSYSTEM_ok = false;
            RCLCPP_ERROR(node_->get_logger(), "[%s] %s (%s)", topic_name_.c_str(), "missing iNAT data", "XCOMParGNSS_LOCKOUTSYSTEM");
        } else if(!par_GNSS_LOCKOUTSYSTEM_ok && !(par_GNSS_LOCKOUTSYSTEM_age_ > xcom_age_max_)) {
            par_GNSS_LOCKOUTSYSTEM_ok = true;
        ...
        if(!parDataIsSet_) {
            if(par_GNSS_LOCKOUTSYSTEM_c_ > (mon_frq_ / par_frq_)) {
                if(par_GNSS_LOCKOUTSYSTEM_age_ == age_max_val_) {
                    par_GNSS_LOCKOUTSYSTEM_age_ = xcom_age_max_ + 1;
                } else {
                    par_GNSS_LOCKOUTSYSTEM_age_++;
                }
                par_GNSS_LOCKOUTSYSTEM_c_ = 0;
            }
        }
