...
NavSatStatus::NavSatStatus(rclcpp_lifecycle::LifecycleNode::SharedPtr node,
    ...
    XComParameters_NavSatStatus(&state),
    ...
    qos_(qos),
    tf2_(tf2) {
    std::thread th(&NavSatStatus::init, this);
    th.detach();
}
...
//                                         + "...").c_str());
        ...
        if(rc == xcom::XComState::ReturnCode::InvalidResponse) {
            if(invalid_channel_) {
//                RCLCPP_INFO(node_->get_logger(), "[%s] %s", topic_name_.c_str(), ("channel " + std::to_string(channel_) + " is in use, trying another one...").c_str());
            }
            channel_++;
            if(channel_ > 31) {
...
void NavSatStatus::handle_response(XCOMResp response) noexcept {
    if(response == XCOMResp::OK) {
        ...
        if(!init_done_) {
            ...
            pub_ = node_->create_publisher<NavSatStatusMsg>(topic_name_, qos_);
            success_ = true;
            // cv_.notify_one();
        }
        init_done_ = true;
...
void NavSatStatus::handle_xcom_param(const XCOMParGNSS_LOCKOUTSYSTEM& param) {
    par_LOCKOUTSYSTEM_age_ = 0;
    setParData(param);
}

void NavSatStatus::activate() {
    active_ = true;
//    RCLCPP_INFO(node_->get_logger(), "%s", ("+ topic " + topic_name_ + " configured and activated").c_str());
    ...
    std::thread th_mon(&NavSatStatus::frq_mon, this);
    th_mon.detach();
}
...
void NavSatStatus::setParData(const XCOMParGNSS_LOCKOUTSYSTEM& param) {
...
void NavSatStatus::updateGNSSSOL(const XCOMmsg_GNSSSOL &msg) {
...
void NavSatStatus::publish() {
    ...
    if(active_) {
        ...
        pub_->publish(navsatstatus_msg_);
